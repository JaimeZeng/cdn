...
update_msg(){
    title="<strong>cdn Update ⚠️</strong>\n\n"
    repo="https://github.com/JaimeZeng/cdn"
    author="$(git log -1 --pretty=format:'%cn')"
    date="$(git log -1 --pretty=format:'%ci')"
    hash="$(git log -1 --pretty=format:'%h')"
    files="$(git log -1 --name-status --format='')" 
    commitLink="https://github.com/JaimeZeng/cdn/commit/$(git log -1 --pretty=format:'%H')"
    file="\n"
    fileLink="https://github.com/JaimeZeng/cdn/blob/main/"
